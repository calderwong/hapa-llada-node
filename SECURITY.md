...
```bash
git status --short
git ls-files | grep -Ei '(^|/)(\.env($|\.)|\.node_token$|.*\.pem$|.*\.key$|id_rsa|id_ed25519|.*credentials\.(json|yaml|yml|toml|ini)|.*secrets\.(json|yaml|yml|toml|ini))' | grep -Ev '(^|/)\.env\.(example|sample|template)$'
```
